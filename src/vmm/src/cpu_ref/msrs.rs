...
pub fn supported_guest_msrs(kvm_fd: &Kvm) -> Result<Msrs> {
    ...
    let mut msrs = Msrs::new(msr_list.as_fam_struct_ref().nmsrs as usize)?;
...
#[cfg(test)]
mod tests {
    use crate::cpu_ref::msrs::{create_boot_msr_entries, supported_guest_msrs};
    use kvm_ioctls::Kvm;
