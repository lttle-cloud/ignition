...
impl MutDevicePio for I8042Wrapper {
    fn pio_read(&mut self, _base: PioAddress, offset: PioAddressOffset, data: &mut [u8]) {
        if data.len() != 1 {
            eprintln!("Invalid I8042 data length on read: {}", data.len());
            return;
        }
        match offset.try_into() {
            ...
            Err(_) => eprintln!("Invalid I8042 read offset."),
        }
    }

    fn pio_write(&mut self, _base: PioAddress, offset: PioAddressOffset, data: &[u8]) {
        if data.len() != 1 {
            eprintln!("Invalid I8042 data length on write: {}", data.len());
            return;
        }
        match offset.try_into() {
            Ok(offset) => {
            ...
            Err(_) => eprintln!("Invalid I8042 write offset"),
        }
    }
}
